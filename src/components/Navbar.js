...
export default function NavComponent() {
    ...
    document.querySelector('nav.navbar.bg-light.fixed-top').classList.remove('darkNav')
    document.querySelector('.navbar-brand').classList.remove('light-logo')
    document.querySelector('#jumbo').classList.remove('light-jumbo')
    Array.from(document.getElementsByClassName('card')).map(card=>card.classList.remove('light-card'))

    } else if(value === 'dark')  {
        setLight(true)

        document.querySelector('.App').style.backgroundColor = `#224b59`;
        document.querySelector('nav.navbar.bg-light.fixed-top').classList.add('darkNav')
        document.querySelector('.navbar-brand').classList.add('light-logo')
        document.querySelector('#jumbo').classList.add('light-jumbo')
        Array.from(document.getElementsByClassName('card')).map(card=>card.classList.add('light-card'))
