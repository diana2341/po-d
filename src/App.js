...
function App() {
  ...
  const skipAction = (action, event) => {
    ...
    if (action === "main") {
      ...
      const focusableElements = introContainer.querySelectorAll('input, select, textarea, button, a[href]');
        ...
        <Suspense fallback={<ReactLoading type={"cubes"} id="loading" />}>
          <Switch>
            <Route
              exact
              path="/"
              render={() => <Home openAccess={openAccess} setOpenAccess={setOpenAccess}/>}
            />
          </Switch>
        </Suspense>
      </Router>
